...
bool compare_desc (std::pair<float,gaol::interval> i, std::pair<float,gaol::interval> j)
{
...
int main()  {
 ...
 //  thrust::device_vector<float>                fbestTag ;
  ...
  // deque<gaol::interval> TempQue ;
   ...
   deque<std::pair<float,gaol::interval>> MainQue ;
   deque<std::pair<float,gaol::interval>> TempQue ;
 ...
 //  fbestTag.push_back(numeric_limits<float>::min()) ;
   ...
   TempQue.push_back(make_pair(PriTemp.left(), x_0));
   TempQue.push_back(make_pair(PriTemp.left(), x_1));
   TempQue.push_back(make_pair(PriTemp.left(), x_2));
   TempQue.push_back(make_pair(PriTemp.left(), x_3));
   TempQue.push_back(make_pair(PriTemp.left(), x_4));
   TempQue.push_back(make_pair(PriTemp.left(), x_5));

  int loop_Counter = 0 ;
   //-- ManageThreads commented until gpu comes alive
   double startComputeTime = omp_get_wtime() ;
   while( (int)TempQue.size()> 0  || (int)MainQue.size()>0 || (int)ManageThreads.size()>0) {
     //  cout << " Idiot!!... Terminate while :)...." << endl ;
       if(USE_GPU==1 && syncFlag == 1 && ManageThreads.size() != 0 && count>=CPU_THRESHOLD) {   // gpu calls to be joined
		   ...
		   //MainQue.clear() ;
		   gpubestbbInt.clear();
		  std::stable_sort(MainQue.begin(), MainQue.end(), compare_desc);
		   for(int i=0; i < (int)gpu_interval_list.size()/dimension; i++) {   // translate gpu return list to gaol
		       for(int j=dimension-1; j>=0; j--) {
				   ...
				   double localPri = gpu_interval_priority[i*dimension + j] ;
				   MainQue.push_front(make_pair(localPri, ij)) ;
		...
		// cout << " TempQue-Size = " << TempQue_priority.size() << endl ;
	     for(int i=0; i<(int)TempQue.size()/dimension; i++) {             // push the TempQue to the MainQue
	        for(int j=0; j < dimension; j++) {
			   ...
			   MainQue.push_back(make_pair(get<0>(TempQue[i*dimension+j]), get<1>(TempQue[i*dimension+j]))) ;
	        }
	     }
	  ...
	  if(USE_GPU==1 && (int)MainQue.size()/dimension - CPU_THRESHOLD > NEW_INTV_THRESHOLD && ManageThreads.size()==0 && gpuCallIntervalTimer>GPU_CALL_INTERVAL && stopGpu==0) {   // minimum number of new intervals to trigger gpu
	//	cout << "Call Interval = " << gpuCallIntervalTimer << endl ;
			    ...
			    for(int i=CPU_THRESHOLD; i<(int)MainQue.size()/dimension; i++) {
			       for(int j=0; j<dimension; j++) {
					  ...
					  gaol::interval local_ij = get<1>(MainQue[i*dimension+j]) ;
					  interval_gpu<double> ij(local_ij.left(), local_ij.right()) ;
			        ...
			        gpu_interval_priority.push_back(get<0>(MainQue[i*dimension+j])) ;
			       }
	   ...
	   for(int i=0; i<dimension; i++) {
	       X.push_back(get<1>(MainQue.front())); MainQue.pop_front(); 
         ...
         float ei = cpu_Func_expr( Xi[i][0].width()/2 + Xi[i][0].left(), Xi[i][1].width()/2 + Xi[i][1].left(), Xi[i][2].width()/2 + Xi[i][2].left(), Xi[i][3].width()/2 + Xi[i][3].left(), Xi[i][4].width()/2 + Xi[i][4].left(), Xi[i][5].width()/2 + Xi[i][5].left() );
             if(ei > fbest) {
			    ...
			    for (int j=0; j< dimension; j++) {
				   ...
				   TempQue.push_back(make_pair(ei,Xi[i][j]));
				}
				// TempQue_priority.push_back(ei) ;
		   }
	   }
